...
#include "jevoisbase/src/Modules/DarknetYOLO/posivar.h"
                    ...
                    public jevois::Parameter<netin>
{
  public: 
      ...
      if (itsPredictFut.valid())
      {
        ...
        if (itsPredictFut.wait_for(std::chrono::milliseconds(5)) == std::future_status::ready)
        {
          ...
          if (success)
          {
            ...
            itsRawInputCv.copyTo(cop);
            cv::findContours(alas , contours, hierarchy, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE, cv::Point(0, 0) );
            /// Draw contours
            cv::Mat drawing = cv::Mat::zeros( itsRawInputCv.size(), CV_8UC3 );
            
           for( int i = 0; i< contours.size(); i++ )
            {
              for(int j=0;j<obj[0].number;j++)
            {
            if(obj[j].h+obj[j].y>480){obj[j].h=(obj[j].h+obj[j].y)-480;}
            if(obj[j].w+obj[j].x>640){obj[j].w=(obj[j].w+obj[j].x)-640;}
            for(int k=0;k<contours[0].size();k++)
            { 
            if(contours[i][k].x<=(obj[j].x+obj[j].w) && contours[i][k].x>=obj[j].x && contours[i][k].y<=(obj[j].y+obj[j].h) && contours[i][k].y>=obj[j].y)
            {cv::drawContours(itsRawInputCv, contours, k, (0,0,255), 2, 8, hierarchy, 0, cv::Point() );
            //cv::drawContours(itsRawInputCv, vector<vector<Point> >(1,contour), -1, color, 1, 8);
             }
            }
            }
            }
          //std::cout<<contours[1][0].x<<"\n";
           for(int k =0;k<contours[0].size();k++)
             {std::cout<<k<<"----- "<<contours[0][k].x<<" "<<contours[0][k].y<<"\n";}
            // for(int i=0;i<obj[0].number;i++)
            // {
            // if(obj[i].h+obj[i].y>480){obj[i].h=(obj[i].h+obj[i].y)-480;}
            // if(obj[i].w+obj[i].x>640){obj[i].w=(obj[i].w+obj[i].x)-640;}
            // cv::Mat ROI(cop,cv::Rect(obj[i].x,obj[i].y,obj[i].w,obj[i].h));
            // cv::Mat inset(itsRawInputCv,cv::Rect(obj[i].x,obj[i].y,obj[i].w,obj[i].h));
            // ROI.copyTo(inset);
            // // ROI.copyTo(itsRawInputCv(cv::Rect(obj[i].x,obj[i].y,obj[i].w,obj[i].h))); 
            // //cop.copyTo(itsRawInputCv(cv::Rect(obj[i].x,obj[i].y,obj[i].w,obj[i].h)));
            // }
            /// Show in a window
            // namedWindow( "Contours", CV_WINDOW_AUTOSIZE );
            // imshow( "Contours", drawing );
            //   ROI.copyTo(itsRawInputCv(cv::Rect(obj[i].x,obj[i].y,obj[i].w,obj[i].h))); 
            //ROI.copyTo(itsRawInputCv(cv::Rect(640,0,640,480)));
	...
	try
	{
	  // Convert input image to RGB for predictions:
	  cv::Mat cvimg = jevois::rawimage::convertToCvRGB(inimg);
	  cv::Mat caneda = jevois::rawimage::convertToCvGray(inimg);
    //caneda.copyTo(alas);
    cv::Canny(caneda, alas, 50, 150, 3, false);
    // cv::Mat ROI(alas,cv::Rect(obj[i]x,y,w,h));
    // ROI.copyTo(cont);
